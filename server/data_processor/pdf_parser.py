...
class FinancialReportParser:
    ...
    def extract_tables_from_pdf(self, pdf_path: str) -> List[pd.DataFrame]:
        """
        从 PDF 文件中提取表格数据 (此功能暂时保留，但新流程不直接使用)
        
        @param pdf_path: PDF 文件路径
        @return: 表格数据列表
        """
        logger.warning("extract_tables_from_pdf 已被标记为不推荐使用，智能分析流程将直接处理纯文本。")
        tables = []
        try:
            with fitz.open(pdf_path) as doc:
...
def main():
    """
    主函数：根据命令行参数执行操作。
    如果提供了文件路径，则提取该文件的纯文本并打印到 stdout。
    否则，演示原始的目录解析功能。
    """
    if len(sys.argv) > 1:
        # 命令行模式：解析单个文件并输出纯文本
        pdf_path = sys.argv[1]
        if not os.path.exists(pdf_path):
            logger.error(f"错误：文件不存在 - {pdf_path}")
            sys.exit(1)
        
        parser = FinancialReportParser()
        text_content = parser.extract_text_from_pdf(pdf_path)
        # 将提取的文本打印到标准输出，Java 程序将捕获它
        print(text_content)

    else:
        # 交互模式或演示模式：解析整个目录
        logger.info("未提供文件路径，进入演示模式...")
        parser = FinancialReportParser()
